...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    print("{:^9}{:^13} ".format("", "") + ' '.join([
        '{:^5}| {:^5}'.format("Won", "Lost") for _ in enumerate(test_agents)]))
...
def my_x3(game, player):
    ...
    my_moves = len(game.get_legal_moves(player))
    opponent_moves = len(game.get_legal_moves(game.get_opponent(player)))
    return float(3 * my_moves - opponent_moves)


def pos_center_dis(game, player):
...
def neg_center_dis(game, player):
...
def pos_player_dis(game, player):
    if game.is_loser(player):
        return float("-inf")
...
def neg_player_dis(game, player):
    if game.is_loser(player):
        return float("-inf")
